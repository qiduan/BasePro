...
var TimerManager = (function (_super) {
     ...
     * @param frameTime
     */
    p.onEnterFrame = function (timeStamp) {
        this._currFrame++;
        this._currTime = egret.getTimer();
        Game.DebugUtils.start("TimerManager:");
        for (var i = 0; i < this._count; i++) {
            var handler = this._handlers[i];
            var t = handler.userFrame ? this._currFrame : this._currTime;
            if (t >= handler.exeTime) {
                Game.DebugUtils.start(handler.method.toString());
                handler.method.call(handler.methodObj, (this._currTime - handler.dealTime) * this._timeScale);
                Game.DebugUtils.stop(handler.method.toString());
        ...
        Game.DebugUtils.stop("TimerManager:");
        return true;
    };
